
<div class="container col-md-12 mx-auto mt-5">
    <div class="row">

        <h2 class="m-5">Ndrysho fjalëkalimin</h2>
        <form class="form-horizontal mx-auto col-12">
            <fieldset>

                <!-- Password input-->
                <div class="form-group">
                    <label class="col-md-4 control-label" for="CurrPass">Vendosni fjalëkalimin aktual</label>
                    <div class="col-md-4">
                        <input id="CurrPass" name="CurrPass" type="password" placeholder="" class="form-control input-md" required="">
                    </div>
                </div>

                <!-- Password input-->
                <div class="form-group">
                    <label class="col-md-4 control-label" for="NewPass">Vendosni një fjalëkalim të ri</label>
                    <div class="col-md-4">
                        <input id="NewPass" name="NewPass" type="password" placeholder="" class="form-control input-md" required="">
                    </div>
                </div>

                <!-- Password input-->
                <div class="form-group">
                    <label class="col-md-4 control-label" for="NewPassRepeat">Rivendosni fjalëkalimin e ri</label>
                    <div class="col-md-4">
                        <input id="NewPassRepeat" name="NewPassRepeat" type="password" placeholder="" class="form-control input-md" required="">
                    </div>
                </div>

                <!-- Button (Double) -->
                <div class="form-group">
                    <label class="col-md-4 control-label" for="Cancel"></label>
                    <div class="col-md-8">
                        <button id="Cancel" name="Cancel" class="btn btn-danger">Anullo</button>
                        <button type="submit" id="Godkend" name="Godkend" class="btn btn-success">Mirato</button>
                    </div>
                </div>
            </fieldset>
        </form>
    </div>
</div>
